...
**Before reporting, pressure-test each finding against its own reachability.** Open the cited code and actively try to prove the issue is *not* a problem — look for the guard, validation, precondition, type constraint, earlier `return`/`throw`, or unreachable branch that would defeat it, and record what you ruled out in `reachability`. Use the result to set severity and confidence honestly: drop a finding the code provably prevents; file genuinely dead or redundant code as a low-severity note rather than a bug; and when reachability depends on runtime or caller context you cannot see statically, keep the finding but set **confidence: low**. The aim is severity and confidence grounded in reachability — not a shorter list.

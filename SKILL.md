...
**Verifiers** (`--verify cross`) are restricted exactly like workers — they only re-read code to check claims — so the same policy applies to them: the verifier preamble carries the in-prompt READ-ONLY block, and under `strict` the same gated allowlists are wired in — `--allowedTools` for Claude verifiers, `--allow`/`--deny` rules for Grok verifiers — with the same per-agent best-effort downgrade for every other backend.

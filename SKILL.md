...
description: >-
  Run parallel multi-agent code review via Solo MCP — replicates the `counselors` tool
  (https://github.com/aarondfrancis/counselors) without depending on `claude -p`. Use when the user
  asks for "counselors", "second opinion", "panel of experts", "parallel review", or "multi-agent
  analysis" — fan the same task out to several AI agents (claude, gemini, codex, copilot,
  antigravity) in parallel and get a synthesized report. Both modes enrich the prompt via a
  repo-discovery + prompt-writing phase (optionally shaped by a preset: bughunt, security,
  regression, contracts, invariants, hotspots, architecture) before fanning out; `--mode run` does
  it inline and fans once, `--mode loop` spawns a durable coordinator that iterates over rounds.
  Do not use for ordinary Solo scratchpad, note, project, or process management merely because the
  user mentions Solo MCP.
...
3. **BUILD-EXECUTION-PROMPT** (core): `user_prompt` (resolve `-f`; fold in `--context`), `prompt_source`, `ENHANCE`, `preset_path`, the prompt-writing text, the execution-boilerplate text, `progress_id`, `duration_ms`. → `execution_prompt` + **`prompt_id`**. When `ENHANCE == on` you run repo discovery here with **Read/Grep/Glob** (you have full file access) **while the workers boot**; cap discovery at min(⅓ × `duration_ms`, 10 min). Discovery cost tracks the size of the repo, not the size of the budget, so a longer run does not mean a longer discovery.
...
5. **COLLECT-WAVE** (core): `workers`, `round_seg = ""`, `extra_tags = []`, `progress_id`, `wave_budget_ms` = the whole `duration_ms` when `verify == off`; when `verify != off`, `duration_ms − verify_reserve_ms` (the core's VERIFY-WAVE *Budget reserve* formula with `reserve_fraction = ⅓` — run's budget pays for one worker wave plus one verify wave), so this drain physically cannot spend the verification slice. No `deadline_timer_id` (run mode lets COLLECT-WAVE arm and poll its own wave-deadline cutoff) and no `on_finish` (run is single-round). Its per-finisher `_any` drain scrapes and closes each worker the instant it idles — fast workers aren't held behind slow ones. → outputs (in memory) + worker `scratchpad_id`s.
6. **VERIFY-WAVE** (core) — **whenever `verify != off`**, including when the step-5 cutoff fired with workers unfinished: the reserve is carved out before the drain starts, so it is still there, and verifying a partial panel beats shipping a `--verify cross` report that was never verified. Pass `findings_by_worker` (the outputs you just collected, keyed by worker), `panel = dispatch`, `verify_mode = verify`, `read_only_policy`, the verifier-preamble text, `progress_id`, `wave_budget_ms = verify_reserve_ms`. Two skips only: VERIFY-WAVE's own check when fewer than 2 workers produced findings (let it make that call), and the pathological case where the whole `duration_ms` is already gone — note that one in progress rather than passing over it silently. → `verdicts` + `verify_ids`. When skipped, both are empty.
7. **SYNTHESIZE** (core): outputs, `agents_csv`, `rounds = 1`, `preset_note` (` · preset {name}` or empty), `verdicts` (from step 6). → `summary_id`.
8. **Finalize** — call **CLASSIFY-AND-FINALIZE** (core): `worker_outcomes` (from COLLECT-WAVE), `cut_short_status` (`partial` if the drain's cutoff fired with workers still unfinished — that cutoff is `duration_ms` with verification off, `duration_ms − verify_reserve_ms` with it on; empty if the drain collected cleanly — run mode has no separate `timeout` exit), `cancelled` (true only if the user interrupted this session mid-run — `stop_process` any live workers first, then SYNTHESIZE what you hold), the worker `scratchpad_id`s, the `verify_ids` (from step 6; empty if verification was off or skipped), `progress_id`, and `prompt_id`. It settles `done`/`partial`/`cancelled`, archives **only** on a clean `done` (so 1 of 3 agents failing while the other 2 succeed is still `done`; verify pads archive alongside the worker pads), and writes the status.
...
The worker preamble enforces this in prompt; under `--read-only strict` additionally pass an agent-specific allowlist via `extra_args` when calling `spawn_agent` for workers. Two backends have one wired in, identified from the agent's `list_agent_tools` entry (`tool_type` plus `command`):

- `tool_type == "claude"` (Claude Code): `["--allowedTools", "Read,Glob,Grep,WebFetch,WebSearch"]`.
- `tool_type == "generic"` whose `command` runs the `grok` binary (xAI Grok Build — Solo reports no dedicated grok `tool_type`, so match the command; other `generic` agents like antigravity get no flags): `["--allow","Read","--allow","Grep","--allow","WebFetch","--deny","Edit","--deny","Bash","--deny","MCPTool"]` — Grok Build's Claude-style permission rules. The `--deny` rules are what actually bind when the agent's base command carries `--always-approve` (a common Solo grok config), since that flag auto-approves anything not explicitly denied.

For any other `tool_type` (`gemini`, `codex`, `copilot`, other `generic`, …) under `strict`, **don't refuse it and don't ask the user** — automatically **downgrade that one agent to best-effort** (the in-prompt READ-ONLY block, which binds every backend), note the per-agent downgrade, and keep going. This preserves the full multi-model panel — the whole point of counselors — while agents with a wired allowlist still get the hard sandbox. So `strict` means "hard sandbox where supported, in-prompt enforcement everywhere else," and a preset defaulting to `strict` (e.g. `bughunt`, `security`) never collapses a mixed panel to the sandboxed backends only.

**Verifiers** (`--verify cross`) are restricted exactly like workers — they only re-read code to check claims — so the same policy applies to them: the verifier preamble carries the in-prompt READ-ONLY block, and under `strict` the same gated flags are wired in — `--allowedTools` for Claude verifiers, `--allow`/`--deny` rules for Grok verifiers, `--sandbox` for Antigravity verifiers — with the same per-agent best-effort downgrade for every other backend. Antigravity verifiers carry the always-on `--dangerously-skip-permissions` regardless of policy, exactly like agy workers.
